...
export default function EmployeeDashboard() {
  ...
  const [mobileTab, setMobileTab] = useState("tasks"); // "tasks" | "activity"
    ...
    const loadDashboard = async () => {
      ...
      try {
        const [
          ...
          axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/admin/getuser", { withCredentials: true }),
          axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/admin/getalltask"),
          axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/admin/getallproject"),
          axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/admin/getattendance"),
          axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/admin/getreports"),
          axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/admin/getannouncements"),
          axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/employee/getsubtask")
        ]);
  ...
  const handletaskstats = () => {
    ...
    const totaltask = tasks.filter(t => String(t.assignedto) === String(user._id))
  ...
  const realNotifications = useMemo(() => {
    ...
    totaltask.forEach(t => {
      ...
      if (due < now) {
        notifs.push({ title: "Task Overdue", sub: t.title, time: getLocalDate(t.dueAt), type: "overdue" });
      } else if (diffHrs <= 24) {
        notifs.push({ title: "Due Soon", sub: t.title, time: "Today", type: "soon" });
      }
    });

    // Add recent activity as notifications
    user?.recentActivity?.slice(0, 2).forEach(r => {
      const tc = tasks.find(t => t._id === r?.refs);
      if (tc) {
        notifs.push({ title: r?.name, sub: tc?.title, time: new Date(r?.time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", hour12: true }), type: "activity" });
      }
    });

    return notifs.slice(0, 5);
  }, [totaltask, user, tasks]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/admin/getattendance");
      setAttendance(
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await axios.get("https://atlasbackend-q7bl.onrender.com/api/v1/admin/getreports");
      setReports(
      ...
      <>
        <div className={styles.mobilePage}>
          ...
          <div className={styles.mobileGreeting}>
            <h2>{getGreeting()}, <strong>{user?.name?.split(" ")[0]}!</strong></h2>
            <div className={styles.mobileMeta}>
              <div className={`${styles.statususer} ${timerStatus === "PUNCH_IN" ? styles.PUNCH_IN : timerStatus === "BREAK" ? styles.BREAK : styles.PUNCH_OUT}`}>
              ...
              <span className={styles.mobileRole}>{user?.designation?.name}</span>
            </div>
          </div>

          {/* Submit standup button */}
          <button
            className={styles.mobileStandupBtn}
            onClick={() => {
              const todayTasksForModal = totaltask.filter(t => t.dueAt && getLocalDate(t.dueAt) === today);
              setModaltasks(todayTasksForModal.map(t => ({ id: t._id, text: t.title || t.name, done: t.status === "Completed" })));
              setNoncompleted(totaltask.filter(t => t?.status !== "Completed"));
              setOverlay(true);
            }}
          >
            <Send size={16} /> Submit Standup Report
          </button>
          <p className={styles.mobileLastSent}>
            <CheckSquare size={13} /> Last sent: Yesterday, 5:45 PM
          </p>

          {/* KPI cards — horizontal scroll */}
          <div className={styles.mobileKpiScroll}>
            {[
              { value: totaltask?.length || 0,      label: "Tasks Assigned" },
              { value: inprogress?.length || 0,     label: "In Progress" },
              { value: pendingtask?.length || 0,    label: "Pending Review" },
              { value: completedtask?.length || 0,  label: "Completed" },
            ].map(({ value, label }) => (
              <div key={label} className={styles.mobileKpiCard} onClick={() => navigate("/employees/tasks")}>
                <h2>{value}</h2>
                <p>{label}</p>
              </div>
            ))}
          ...
          <div className={styles.mobileTabBar}>
            <button
              className={`${styles.mobileTabBtn} ${mobileTab === "tasks" ? styles.mobileTabActive : ""}`}
              onClick={() => setMobileTab("tasks")}
            >
              <Calendar size={15} /> Today's Tasks
            </button>
            <button
              className={`${styles.mobileTabBtn} ${mobileTab === "activity" ? styles.mobileTabActive : ""}`}
          ...
          {mobileTab === "tasks" ? (
            <div className={styles.mobileTabContent}>
              {todayTasks.length === 0 && (
                <p className={styles.mobileEmpty}>No tasks due today.</p>
              )}
              {todayTasks.map(t => (
                <TaskRow key={t._id} task={{ ...t, project: projects.find(p => p._id === t.projectId) }} />
              ))}
              <button className={styles.addTaskBtn} onClick={() => setTask(true)}>+ Add New Task</button>
            </div>
          ) : (
            <div className={styles.mobileTabContent}>
              {user?.recentActivity?.map((r, i) => {
                const tc = tasks.find(t => t._id === r?.refs);
                return (
                  <div key={i} className={styles.mobileActivityRow}>
                    <span className={styles.activityDot}></span>
                    <div className={styles.mobileActivityInfo}>
                      <p><strong>{r?.name}</strong> — {tc?.title}</p>
                      <span>{new Date(r?.time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", hour12: true })}</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div className={styles.mobileSectionHeader}>
            <Megaphone size={18} /><h3>Announcements</h3>
          ...
          <div className={styles.mobileSectionHeader}>
            ...
            <span className={styles.mobileSeeAll}>See All</span>
          </div>
          <div className={styles.mobileSectionCard}>
            {realNotifications.length === 0 ? (
              <p style={{ fontSize: "12px", color: "#6E768F", padding: "8px 0" }}>No new notifications</p>
            ) : (
              realNotifications.map((n, i) => (
                <NotifRow key={i} title={n.title} sub={n.sub} time={n.time} type={n.type} />
              ))
            )}
          </div>

          {/* Charts row */}
          <div className={styles.mobileChartsRow}>
            <div className={styles.mobileChartCard}>
              <h4>Productivity Trend</h4>
              <ProductivityChart data={productivityData} compact />
            </div>
            <div className={styles.mobileChartCard}>
              <h4>Activity Snapshot</h4>
              <ActivityHeatmap heatmapData={heatmapData} reports={reports} compact />
              <div className={styles.legendCompact}>
                <div className={styles.restdiv}><span className={styles.grey}></span>Rest</div>
                <div className={styles.restdiv}><span className={styles.green}></span>Good</div>
                <div className={styles.restdiv}><span className={styles.darkgreen}></span>Great</div>
              </div>
            </div>
          </div>
        ...
        <DailyReportModal open={overlay} onClose={() => setOverlay(false)} user={user} tasks={modaltasks} allTasks={noncompleted} allTasksSubtasks={subtask} />
          ...
          <div className={styles.leftHeader}>
            ...
            <div className={styles.userText}>
              ...
              <div className={styles.userMeta}>
                <div
                  className={`${styles.statususer} ${timerStatus === "PUNCH_IN"
                    ? styles.PUNCH_IN
                    : timerStatus === "BREAK"
                      ? styles.BREAK
                      : styles.PUNCH_OUT
        ...
        <div className={styles.dashboardMain}>
          ...
          <div className={styles.mainRight}>
            ...
            <div className={`${styles.infoCard} ${styles.hoverCard}`}>
              <h3>Notifications</h3>
              {realNotifications.length === 0 ? (
                <p style={{ fontSize: "13px", color: "#6E768F", padding: "8px 0" }}>No new notifications</p>
              ) : (
                realNotifications.map((n, i) => (
                  <NotifRow key={i} title={n.title} sub={n.sub} time={n.time} type={n.type} />
                ))
...
function ActivityHeatmap({ heatmapData, reports, compact }) {
...
function ProductivityChart({ data, compact }) {
  return (
    <div className={compact ? styles.chartWrapperCompact : styles.chartWrapper}>
      <ResponsiveContainer width="100%" height={compact ? 110 : 200}>
        <AreaChart data={data}>
          <defs>
...
function TaskRow({ task }) {
  ...
  const handleComplete = async () => {
    ...
    try {
      await axios.post(
        "https://atlasbackend-q7bl.onrender.com/api/v1/employee/completedtask",
        { taskid: task._id },
        { withCredentials: true }
      );
...
function NotifRow({ title, sub, time }) {
  return (
