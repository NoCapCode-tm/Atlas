...
const [showModal, setShowModal] = useState(false);
    ...
    const fetchEmployee = async () => {
      try {
        const res = await axios.get(
          `https://atlasbackend-1bt5.onrender.com/api/v1/admin/getuserdetails/${id}`,
  ...
  //           `https://atlasbackend-1bt5.onrender.com/api/v1/manager/getaisummary`,
  //           { id: employee._id },
  //           { withCredentials: true }
  //         );
...
const updateTaskStatus = async (taskId, newStatus) => {
  try {
    await axios.put(
      `https://atlasbackend-1bt5.onrender.com/api/v1/employee/updatetask/${taskId}`,
      { status: newStatus },
      { withCredentials: true }
    );
